...
In addition to the training notebooks, this project provides a simple **local web demo** built with **Streamlit**.  
The demo allows users to upload or drag-and-drop a chart image, enter a Vietnamese question, choose one of the supported model configurations, and generate an answer directly in the browser.

The local demo supports four configurations:

| Model | Description | Source |
|------|-------------|--------|
| **A1** | Custom ChartVQA model with LSTM decoder | Hugging Face checkpoint |
| **A2** | Custom ChartVQA model with Transformer decoder | Hugging Face checkpoint |
| **B1** | Qwen2-VL-2B-Instruct zero-shot baseline | Loaded from Hugging Face |
| **B2** | Qwen2-VL-2B-Instruct fine-tuned with LoRA/QLoRA | Hugging Face LoRA adapter |

> **Note:** B1 is a zero-shot model. It does not require any fine-tuned checkpoint. The prompt is hidden inside the application code and is automatically applied during inference.

---

### 9.4.1 Install Dependencies

It is recommended to create a clean Python environment before running the demo.

#### Option 1: Using Conda

```bash
conda create -n vichartvqa-demo python=3.10 -y
conda activate vichartvqa-demo
```
#### Option 2: Using venv
```
python -m venv vichartvqa-demo
vichartvqa-demo\Scripts\activate
```
Then install the required packages:
```
pip install -U streamlit pillow numpy pandas
pip install -U torch torchvision torchaudio
pip install -U transformers datasets huggingface_hub accelerate peft sentencepiece protobuf
pip install -U qwen-vl-utils
```
If the machine has an NVIDIA GPU and the user wants to run B1/B2 more efficiently, install **bitsandbytes**:
```
pip install -U bitsandbytes
```
On Windows, bitsandbytes support may depend on the CUDA/PyTorch environment. If 4-bit loading fails, the application will fall back to normal float16 or CPU-compatible loading where possible.

### 9.4.2 Hardware Recommendation

The demo can run on CPU for A1 and A2, but B1 and B2 are much larger because they are based on Qwen2-VL.

| Model | CPU | GPU Recommended | Notes |
|------|------|-----------------|-------|
| A1 | Supported | Optional | Lightweight compared to Qwen2-VL |
| A2 | Supported | Optional | Slightly heavier than A1 |
| B1 | Very slow | Recommended | Loads full Qwen2-VL base model |
| B2 | Very slow | Recommended | Loads Qwen2-VL base model + LoRA adapter |

Recommended GPU:

| Use Case                | Recommended VRAM |
| ----------------------- | ---------------- |
| A1 / A2 inference       | 4–8 GB           |
| B1 zero-shot inference  | ≥ 8 GB           |
| B2 fine-tuned inference | ≥ 8–16 GB        |
| Smooth B1/B2 demo       | ≥ 16 GB          |

If CUDA is available, the application automatically uses GPU:
```
torch.device("cuda" if torch.cuda.is_available() else "cpu")
```

Users can verify GPU availability with:
```
...
```
This will start a local web server and automatically open the demo in your browser.

The interface contains:

1. **Image upload area**
Users can upload or drag-and-drop a chart image.
2. **Question input box**
Users enter a Vietnamese question about the uploaded chart.
3. **Model selector**
Users choose one of the following:

| Model | Description |
|------|-------------|
| **A1** | Custom ChartVQA model with LSTM decoder |
| **A2** | Custom ChartVQA model with Transformer decoder |
| **B1** | Qwen2-VL-2B-Instruct zero-shot baseline |
| **B2** | Qwen2-VL-2B-Instruct fine-tuned with LoRA/QLoRA |
4. **Generate Answer button**
The selected model generates a Vietnamese answer.

### 9.4.4 First Run Behavior

On the first run, the demo automatically downloads required models from Hugging Face:

| Component                 | Hugging Face Source                    |
| ------------------------- | -------------------------------------- |
| A1 checkpoint             | `Zenng2812/vichartvqa-a1-lstm`         |
| A2 checkpoint             | `Zenng2812/vichartvqa-a2-transformer`  |
| B1 base model             | `Qwen/Qwen2-VL-2B-Instruct`            |
| B2 LoRA adapter           | `Zenng2812/vichartvqa-b2-qwen2vl-lora` |
| Dataset vocabulary source | `Zenng2812/vqa-vietnamese-charts`      |
| Text encoder              | `vinai/phobert-base`                   |
| Image encoder             | `google/vit-base-patch16-224-in21k`    |

The first launch may take several minutes depending on internet speed and hardware.
After the first download, Hugging Face caches the files locally, so later runs are faster.

### 9.4.5 How Each Model is Loaded

#### A1 and A2

A1 and A2 are custom PyTorch models. Their checkpoints are downloaded from Hugging Face using:
```
hf_hub_download(...)
```
Then the **.pth** checkpoint is loaded into the custom **ChartVQAModel** architecture.

Because A1/A2 use a fixed answer vocabulary, the application reconstructs the answer vocabulary from the training split of the dataset before loading the checkpoint.

#### B1
B1 is a zero-shot baseline. It loads the original Qwen2-VL model directly:
```
Qwen/Qwen2-VL-2B-Instruct
```
No fine-tuned weights are used.
The model receives the uploaded image and user question through a hidden prompt template inside the application.
#### B2

B2 loads the same **Qwen2-VL base model** as B1, then applies the **fine-tuned LoRA adapter**:

```
Zenng2812/vichartvqa-b2-qwen2vl-lora
```

This allows the model to keep the general multimodal ability of Qwen2-VL while adapting its answer style to Vietnamese chart VQA.

#### 9.4.6 GPU and Memory Notes

For B1 and B2, the application attempts to use 4-bit loading through bitsandbytes when CUDA is available:
...
```
If bitsandbytes is unavailable or CUDA is not detected, the application may fall back to normal model loading. This can require more RAM/VRAM and may be significantly slower.

To reduce memory usage:

* Close other GPU-heavy applications.
* Use A1 or A2 if the machine has limited VRAM.
* Use B1/B2 only on a CUDA-enabled machine if possible.
* Clear the Streamlit model cache from the sidebar if switching models repeatedly.
* Restart the Streamlit server if GPU memory is not released properly.